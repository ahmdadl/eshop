            ...
            <{{ $cpt ?? 'home' }} ref="childCmp">
                <template v-slot="h">
                    <ul class="nav justify-content-end bg-dark text-light col-12">
                        ...
                        <li class="nav-item">
                            @if(app()->isLocale('en'))
                                <a class="nav-link text-light" rel="alternate" hreflang="ar" href="/ar{{
                        Str::after(url()->current(), LaravelLocalization::getCurrentLocale())
                    }}">العربية
                                </a>
                            @else
                                <a class="nav-link text-light" rel="alternate" hreflang="en" href="/en{{
                        Str::after(url()->current(), LaravelLocalization::getCurrentLocale())
                    }}">English
                                </a>
                            @endif
                        ...
                        <div class="container">
                            ...
                            <ul class="unstyled-list">
                                <li class="nav-item dropdown float-left d-inline text-light d-md-none">
                                    <a id="cartDropdownList" class="nav-link dropdown-toggle text-light" href="#"
                                        role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                        <span v-if="h.d.cartLoader" class="spinner-grow bg-light" role="status"
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right" aria-labelledby="cartDropdownList" style="width: 27rem;    overflow-y: scroll;
                                    max-height: 57vh;">
                                        <ul class="list-unstyled">
                                            <li class="media border-bottom py-1" v-for="c in h.d.cart">
                            ...
                            <div class="collapse navbar-collapse" id="navbarSupportedContent">
                                ...
                                <form action="/{{ app()->getLocale() }}/p/ser" method="GET"
                                    class="form form-inline col-md-9">
                                    <div class="form-group col-8">
                                        <input type="search" class="form-control col-12" name="q"
                                ...
                                <ul class="navbar-nav ml-auto">
                                    ...
                                    <li class="nav-item dropdown d-none d-md-block">
                                        ` <a id="wishDropdown" class="nav-link dropdown-toggle" href="#" role="button"
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right" aria-labelledby="wishDropdown"
                                            style="width: 27rem;    overflow-y: scroll;
                                        max-height: 57vh;">
                                            <ul class="list-unstyled">
                                                <li class="media border-bottom py-1" v-for="c in h.d.cart">
                                                    <img :src="'/img/' + c.product.p_cat.parent.slug + '/' + c.product.img[0]"
                    ...
                    @if(isset($cats))
                        <ul class="nav nav-taps nav-fill navbar-light" style="background-color: #e3f2fd" id="myTab">
                            <li class="nav-item">
                                    ...
                                    <div class="dropdown-menu">
                                        @foreach($c->subCat as $sc)
                                            <a class="dropdown-item"
                                                ...
                                                @if($cpt==='product' )
                                                v-on:click.prevent="$refs.childCmp.loadData('{{ $sc->slug }}', null,
                                                '{{ '/' .app()->getLocale() .'/c/'. $c->slug }}')"
                                        @endif>
                                        {{ $sc->name }}
                                        </a>
                            @endforeach
...
<script type="text/javascript"
    src="https://cdn.jsdelivr.net/npm/bootstrap.native@2.0.27/dist/bootstrap-native-v4.min.js" defer>
</script>
